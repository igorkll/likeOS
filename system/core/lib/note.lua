...
function note.midi(n)
    if type(n) == "string" then
        ...
        if tonumber(notes[n]) ~= nil then
            ...
            error("Wrong input " ..
            tostring(n) .. " given to note.midi, needs to be <note>[semitone sign]<octave>, e.g. A#0 or Gb4")
        end
...
function note.freq(n)
    if type(n) == "string" then
        ...
        if tonumber(notes[n]) ~= nil then
            ...
            error(
            "Wrong input " ..
            tostring(n) .. " given to note.freq, needs to be <note>[semitone sign]<octave>, e.g. A#0 or Gb4", 2)
        end
    elseif type(n) == "number" then
